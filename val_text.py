...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_map = gt_map *self.dense_norm
        
        sample = {'image':image,'dots':dots, 'boxes':boxes, 'pos':rects, 'gt_map':gt_map, 'scale':scale, 'text': text}
        return sample['image'], sample['dots'], sample['boxes'], sample['pos'] ,sample['gt_map'],im_id,sample['scale'], sample['text']

def collate_fn(batch):
    images = [item[0] for item in batch]
    dots = [torch.from_numpy(item[1]) for item in batch]
    boxes = [item[2] for item in batch]
    pos = batch[0][3]
    gt_map = [item[4] for item in batch]
    im_id = [item[5] for item in batch]
    scale = [item[6] for item in batch]
    texts = [item[7] for item in batch]
    
    images = torch.stack(images, dim=0)
    dots = torch.stack(dots, dim=0)
    boxes = torch.stack(boxes, dim=0)
    gt_map = torch.stack(gt_map, dim=0)
    scale = torch.stack(scale, dim=0)

    return images, dots, boxes, pos, gt_map, im_id, scale, texts

def val_func(model,device,args=None, dataset='val'):
    ...
    for data_iter_step, (samples, gt_dots, _, pos, gt_map,im_id,_, text) in enumerate(metric_logger.log_every(data_loader_test, print_freq, header)):
        ...
        if s_cnt >= 1:
            ...
            for r_image in r_images:
                ...
                with torch.no_grad():
                    while start + 383 < w:
                        input_x = [r_image[:,:,:,start:start+384], text]
                ...
                pred_cnt += torch.sum(density_map/args.dense_norm).item()
            pred_cnt_new = 0
            density_map = torch.zeros([h,w])
            density_map = density_map.to(device, non_blocking=True)
            start = 0
            ...
            pred_cnt_new = torch.sum(density_map/args.dense_norm).item()
            ...
            for rect in pos:
                ...
                if cnt>3:
                    break
                e_cnt += torch.sum(density_map[rect[0]:rect[2]+1,rect[1]:rect[3]+1]/args.dense_norm).item()
            e_cnt = e_cnt / 3
            if e_cnt > 1.8:
                pred_cnt_new /= e_cnt
    ...
    print(f'{dataset}: ' + 'Current MAE: {:5.2f}, RMSE: {:5.2f} '.format( train_mae/(len(data_loader_test)), (train_rmse/(len(data_loader_test)))**0.5))
    return mae, mse
