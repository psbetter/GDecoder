...
def load_model_FSC(args, model_without_ddp):
    if args.resume:
        ...
        checkpoint_bert = torch.load("/data/ps/research/checkpoints/groundingdino_swinb_cogcoor.pth", map_location='cpu')
        for key in checkpoint_bert.keys():
            if "bert" in key:
                checkpoint['model'][key] = checkpoint_bert[key]
