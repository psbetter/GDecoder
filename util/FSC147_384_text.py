...
class resizeTrainImage(object):
    ...
    def __call__(self, sample):
        ...
        image, lines_boxes, density, dots, im_id, m_flag, text = sample['image'], sample['lines_boxes'],sample['gt_density'], sample['dots'], sample['id'], sample['m_flag'], sample['text']
...
def collate_fn(batch):
    ...
    gt_density = [item[1] for item in batch]
    boxes = [item[2] for item in batch]
    
    m_flag = [item[3] for item in batch]
    scale = [item[4] for item in batch]
    ...
    boxes = torch.stack(boxes, dim=0)
    scales = torch.stack(scale, dim=0)
    # m_flag = torch.stack(m_flag, dim=0)

    return images, gt_density, boxes, m_flag, scales, texts
