...
from util.FSC147_384_text import TransformTrain, collate_fn
import models.GDecoder as CountNet
from val_text import val_func

from util.losses import kl, l2

def get_args_parser():
    parser = argparse.ArgumentParser('GDecoder training', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='model_gdecoder', type=str, metavar='MODEL',
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/data/ps/research/datasets/FSC-147/', type=str,
                        help='dataset path')

    parser.add_argument('--output_dir', default='/data/ps/research/GDeocder/results_gdecoder',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='/data/ps/research/GDeocder/results_gdecoder',
    ...
    parser.add_argument('--dense_norm', type=int, default=1,
                        help='density transform')
...
data_path = '/data/ps/research/datasets/FSC-147/'
...
class TrainData(Dataset):
    def __init__(self):
        ...
        self.class_dict = {}
        with open(args.data_path + 'ImageClasses_FSC147.txt') as f:
            for line in f:
                key = line.split()[0]
                val = line.split()[1:]
    ...
    def __getitem__(self, idx):
        ...
        return sample['image'], sample['gt_density'], sample['boxes'], sample['m_flag'], sample['scale'], sample['text']
...
def main(args):
    ...
    # define the model
    model = CountNet.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, _, m_flag, _, text) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            with torch.cuda.amp.autocast():
                inputx = [samples, text, gt_density]
                output, dist_i, dist_d, dist_ref, output_i, output_d, gt_map_scaled, z_i, z_d = model(inputx)

            # Compute loss function
            kl_loss = kl(dist_i, dist_ref) + kl(dist_d, dist_ref) + kl(dist_i, dist_d)
            ...
            loss = (output_i - gt_map_scaled) ** 2 + (output_d - gt_map_scaled) ** 2

            loss = (loss * masks / (384*384)).sum() / output.shape[0] + 1e-3 * kl_loss + 0.01 * l2_loss

            metric_logger.update(kl=kl_loss.item())
            metric_logger.update(l2=l2_loss.item())
            ...
            for i in range(output.shape[0]):
                pred_cnt = torch.sum(output[i]/args.dense_norm).item()
                pred_cnt_list.append(pred_cnt)
                output_list.append(output[i])
                gt_cnt = torch.sum(gt_density[i]/args.dense_norm).item()
        ...
        if args.output_dir and epoch >= 100 and epoch % 5 == 0:
            mae_new,mse_new = val_func(model=model,device=device, args=args)
            if mae_new<mae:
                mae = mae_new
                mse = mse_new
                misc.save_model(
                ...
                mae_test,mse_test = val_func(model=model,device=device,args=args,dataset='test')
